...
```diff
- import VietmapGL from "@vietmap/vietmap-gl-react-native";
+ import * as MapLibreRN from "@vietmap/vietmap-gl-react-native";

- <VietmapGL.MapView />
+ <MapLibreRN.MapView />
```

Any namespace can be chosen, but using `VietmapGL` is not recommended anymore due to risk of confusion.
...
```diff
- import { locationManager, offlineManager, snapshotManager } from "@vietmap/vietmap-gl-react-native";
+ import { LocationManager, OfflineManager, SnapshotManager } from "@vietmap/vietmap-gl-react-native";
```
